...
class EventAnalyzer {
      ...
      if (this._schemas[schema_id]) {
        ...
        try {
          ...
          if (callback) {
            ...
            callback({
              success: true,
              errors: { ...this._schemas[schema_id].errors }
            })
          }
        } catch (e) {
          if (callback) {
            // eslint-disable-next-line standard/no-callback-literal
            callback({ success: false, errors: null })
          }
        }
  ...
  getSchema(schema) {
    ...
    if (schema_cache) {
      ...
      // schema is missing, lets compile and return
      this._schemas[schema_id] = {
        compiled: this.ajv.compile(schema),
        schema: schema
      }
  ...
  fetchComponentInfo(
    ...
    if (components_registered) {
      ...
      return StateManager.StateManager.getInstance().getManager(component_id)
    }
  }
  ...
  validateEventAction(event_action) {
    if (event_action.input === undefined) {
      event_action.input = []
    }
    if (event_action.output === undefined) {
      event_action.output = []
    }
    try {
      const report = this.validateEventActionRun(
        event_action,
        event_action.input[0],
        event_action.output[0]
      )
      return report
    } catch (report) {
      return report
  ...
  validateEventActionRun(
    ...
    if (component === undefined) {
      ...
      report.data = {
        ...
        name: 'missing',
        was: 'section',
        use: Object.keys(Manager.ComponentManager.getInstance().getComponents())
      }
      throw report
    } // total failure
    const component_inst = this.fetchComponentInfo(
      component.id,
      components_registered,
      components_inventory
    )
    if (component_inst === undefined) {
      report.error_resolve =
        "Make sure state manager registers '" + component.id + "'"
      report.error_detail =
        "Component '" + component.id + "' was not registered with state manager"
      report.data = {
        type: 'critical',
        name: 'missing',
        was: component.id,
    ...
    const component_type = Manager.ComponentManager.getInstance().getComponent(
      component_inst.props.type
    )
    if (component_type === undefined) {
      report.error_resolve =
        'Use one of these: ' +
        Manager.ComponentManager.getInstance().getComponentTypes()
      report.error_detail =
        "Component type '" + component_inst.props.type + "' is not supported"
      report.data = {
        type: 'critical',
        name: 'unsupported',
        was: component_inst.props.type,
        use: Manager.ComponentManager.getInstance().getComponentTypes()
      }
      throw report
    } // total failure
    ...
    const trigger_inst = this.fetchComponentInfo(
      trigger.id,
      components_registered,
      components_inventory
    )
    if (trigger_inst === undefined) {
      report.error_resolve =
        "Make sure state manager registers '" + trigger.id + "'"
      report.error_detail =
        "Trigger '" + trigger.id + "' was not registered with state manager"
      report.data = {
        type: 'critical',
        name: 'missing',
        was: trigger.id,
    ...
    const trigger_type = Manager.ComponentManager.getInstance().getComponent(
      trigger_inst.props.type
    )
    if (trigger_type === undefined) {
      report.error_resolve =
        ...
        Manager.ComponentManager.getInstance().getComponentTypes()
      report.error_detail =
        "Trigger type '" + trigger_inst.props.type + "' is not supported"
      report.data = {
        type: 'critical',
        name: 'unsupported',
    ...
    if (component_type && component_type.events) {
      ...
      if (!component_event) {
        // referred event does not exist
        report.error_resolve =
          'Use one of these ' + Object.keys(component_type.events)
        report.error_detail =
          "Component '" +
          component.id +
          "' does not have event '" +
      ...
      if (trigger_type && trigger_type.triggers && component_event) {
        ...
        if (!triggers_event) {
          report.error_resolve =
            'Use one of these ' + Object.keys(trigger_type.triggers)
          report.error_detail =
            "Component '" +
            trigger.id +
        ...
        const component_data_validated = this.validateData(
          component_event.schema,
          data_input
        )
        if (component_data_validated.valid === false) {
          report.error_resolve =
            'Must conform to schema ' +
            JSON.stringify(component_event.schema.$id)
          report.error_detail =
            "Component '" +
            component.id +
            "' event '" +
            component.event +
            "' was provided with bad input data " +
            JSON.stringify(data_input) +
            ''
          report.data = {
            type: 'data',
            name: component.event,
        ...
        if (typeof event.transform === 'string') {
          event_transform = Event.getTransformFunction(event)
        }
        ...
        try {
          ...
          const trigger_data_validated = this.validateData(
            triggers_event.schema,
            transformed_data
          )
          if (trigger_data_validated.valid === false) {
            report.error_resolve =
              'Must conform to schema ' +
              JSON.stringify(triggers_event.schema.$id)
            report.error_detail =
              "Component '" +
              trigger.id +
              "' action '" +
              trigger.action +
              "' was provided with bad input data " +
              JSON.stringify(transformed_data) +
              ''
            report.data = {
              type: 'data',
              name: trigger.action,
          ...
          if (data_output !== undefined) {
            if (compareDeep(data_output, transformed_data) === false) {
              ...
              report.error_detail =
                "Whitebox '" +
                component.id +
                "' testing got unexpected result with regard to input"
              report.data = {
                type: 'test',
                name: trigger.action,
                was: transformed_data,
